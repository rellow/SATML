...
源材料确认的核心安全缺陷为“OTA 固件远程获取链”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`Go2_OTA固件远程获取链`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
