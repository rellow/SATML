...
# G2-015 BLE 握手零熵与授权态残留

## 1. 一句话结论

BLE 握手零熵与授权态残留。结论已有真机或真实服务闭环证据。
...
源材料确认的核心安全缺陷为“BLE 握手零熵与授权态残留”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：未动态闭环 12、BLE 真机链。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
