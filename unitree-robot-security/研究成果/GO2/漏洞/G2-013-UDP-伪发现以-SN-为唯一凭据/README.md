...
# G2-013 UDP 伪发现以 SN 为唯一凭据

## 1. 一句话结论

UDP 伪发现以 SN 为唯一凭据。结论已有真机或真实服务闭环证据。
...
证据索引见 `证据/材料清单.md`。源材料：未动态闭环 10、零凭证控制闭环。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
