...
# G2-003 BLE 配网注入与网络劫持

## 1. 一句话结论

BLE 配网注入与网络劫持。当前仅有候选线索或关键运行时条件尚未确认。
...
源材料确认的核心安全缺陷为“BLE 配网注入与网络劫持”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`Go2_BLE配网注入劫持网络`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
