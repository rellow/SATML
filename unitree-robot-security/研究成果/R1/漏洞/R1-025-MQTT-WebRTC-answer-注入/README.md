...
# R1-025 MQTT WebRTC answer 注入

## 1. 一句话结论

MQTT WebRTC answer 注入。原始假设已被后续实验或可达性复核推翻；保留失败过程用于防止重复研究。
...
源材料确认的核心安全缺陷为“MQTT WebRTC answer 注入”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`AUD-p3-mqtt-webrtc-answer-injection`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
