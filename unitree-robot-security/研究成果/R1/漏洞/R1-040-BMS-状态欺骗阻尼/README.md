...
BMS 状态欺骗阻尼。代码路径和根因已确认，但尚未完成无歧义的真机闭环。
...
源材料确认的核心安全缺陷为“BMS 状态欺骗阻尼”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`AUD-r3-batguard-bmsstate-spoof-damp`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
