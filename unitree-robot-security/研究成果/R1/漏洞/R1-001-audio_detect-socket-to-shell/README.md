...
# R1-001 audio_detect socket-to-shell

## 1. 一句话结论

audio_detect socket-to-shell。结论已有真机或真实服务闭环证据。
...
源材料确认的核心安全缺陷为“audio_detect socket-to-shell”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`R1_audio_detect网络rce`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
