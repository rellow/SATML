...
# R1-042 robot_type 自复制耗尽文件系统

## 1. 一句话结论

robot_type 自复制耗尽文件系统。代码路径和根因已确认，但尚未完成无歧义的真机闭环。
...
证据索引见 `证据/材料清单.md`。源材料：`AUD-robottype-selfcopy-fs-exhaustion`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
