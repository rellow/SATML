...
# R1-021 KVS SDP 属性名堆溢出

## 1. 一句话结论

KVS SDP 属性名堆溢出。代码路径和根因已确认，但尚未完成无歧义的真机闭环。
...
`静态确认`。迁移裁决：修正上游修复版本与 broker 可达性。
...
证据索引见 `证据/材料清单.md`。源材料：`AUD-kvsmem-sdp-attr-overflow`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
