...
# R1-010 gpt-proxy 以 SN 认证

## 1. 一句话结论

gpt-proxy 以 SN 认证。结论已有真机或真实服务闭环证据。
...
`动态确认`。迁移裁决：完整 SN 脱敏，保留阴性影响结论。

## 4. 攻击前提

攻击者需要能够访问对应公开云端接口；任何账户、SN 或令牌前提均不得用真实值提交。

## 5. 根本原因

源材料确认的核心安全缺陷为“gpt-proxy 以 SN 认证”；根因位于输入信任边界、权限分离或安全状态校验不足。
...
证据索引见 `证据/材料清单.md`。源材料：`云端WSS_gpt-proxy_SN即凭证`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
