...
# R1-006 chat_go 知识库路径穿越写入

## 1. 一句话结论

chat_go 知识库路径穿越写入。结论已有真机或真实服务闭环证据。
...
`动态确认`。迁移裁决：作为 R1-CHAIN-002 投放原语。
...
证据索引见 `证据/材料清单.md`。源材料：`chat_go知识库任意文件写RCE`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
