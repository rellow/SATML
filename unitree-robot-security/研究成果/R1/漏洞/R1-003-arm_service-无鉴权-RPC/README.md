...
# R1-003 arm_service 无鉴权 RPC

## 1. 一句话结论

arm_service 无鉴权 RPC。结论已有真机或真实服务闭环证据。
...
`动态确认`。迁移裁决：作为 R1-CHAIN-001 第一阶段。
...
证据索引见 `证据/材料清单.md`。源材料：`arm服务rename穿越RCE链`。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
