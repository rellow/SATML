...
`动态确认`。迁移裁决：合并为一个权威报告，不提交有效凭据或验证日志原值。
...
长期凭据被硬编码、共享、明文记录或与设备身份错误绑定，导致秘密边界失效。
...
证据索引见 `证据/材料清单.md`。源材料：AWS、DashScope/博查及“硬编码凭证全线泄露”三个重复包。未提交的大型或敏感证据统一登记在仓库根目录 `材料清单/未提交材料.csv`。
